...
public class TwitterFormActionTest extends StrutsTestCase {
    @Override
    ...
    public void test_execute_returns_success() throws ServletException, UnsupportedEncodingException {
        ...
        String body = executeAction("/guests/twitterForm.action");
        ...
        assertEquals("/WEB-INF/content/guests/twitterForm.jsp", response.getForwardedUrl());
    ...
    public void test_execute_redirect_back_to_nickname_if_missing() throws Exception {
        ActionProxy actionProxy = getActionProxy("/guests/twitterForm.action");
        TwitterFormAction action = (TwitterFormAction) actionProxy.getAction();
    ...
    public void test_execute_redirect_back_to_country_if_nickname_present_country_missing() throws Exception {
        ActionProxy actionProxy = getActionProxy("/guests/twitterForm.action");

        GuestForm guestFormInSession = new GuestForm();
        guestFormInSession.setNickname("New Nickname");
        actionProxy.getInvocation().getInvocationContext()
                .setSession(new HashMap(Map.of("guest", guestFormInSession)));

        TwitterFormAction action = (TwitterFormAction) actionProxy.getAction();
        String result = actionProxy.execute();

        assertEquals("backToCountry", result);
        assertEquals(302, response.getStatus());
    }
